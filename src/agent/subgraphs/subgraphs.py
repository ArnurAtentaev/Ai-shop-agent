...
from src.agent.nodes.crud_nodes import (
    tool_find_product,
    tool_find_similar,
...
from src.agent.nodes.common_nodes import tool_base_answer_node, model_fallback_node
from src.agent.nodes.insert_nodes import insert_confirm_node
...
def search_nodes_subgraph():
    ...
    search_sub_workflow.add_node("base_answer_node", tool_base_answer_node)
    search_sub_workflow.add_node("model_fallback", model_fallback_node)
...
def insert_nodes_subgraph():
    ...
    insert_sub_workflow.add_node("insert_confirm_node", insert_confirm_node)
    insert_sub_workflow.add_node("model_fallback", model_fallback_node)
...
def general_actions_subgraph():
    general_sub_workflow = StateGraph(OverallAgentState)

    GENERAL_TOOLS = {
        "get_orders": tool_get_orders,
        "general_question": tool_general_questions,
    }

    for name, func in GENERAL_TOOLS.items():
        general_sub_workflow.add_node(name, func)

    general_sub_workflow.add_node("base_answer_node", tool_base_answer_node)
    general_sub_workflow.add_node("model_fallback", model_fallback_node)
